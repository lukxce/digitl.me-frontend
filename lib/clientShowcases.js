...
function showcaseFetchQuery(extra = {}) {
  return {
    ...
    "populate[category][fields][2]": "label",
    "populate[categories][fields][0]": "name",
    "populate[categories][fields][1]": "title",
    "populate[categories][fields][2]": "label",
    ...extra,
  };
}
...
function pickCategoryValue(value) {
  ...
  const record = value.data?.attributes ?? value.data ?? value.attributes ?? value;

  if (Array.isArray(record)) {
    const labels = record.map((item) => pickCategoryValue(item)).filter(Boolean);
...
function pickSuccessRateTitle(record) {
  const keys = ["title", "Title", "heading", "headline", "value", "metric", "name"];
  for (const key of keys) {
...
function normalizeSuccessRate(raw) {
  ...
  return successRateEntries(value).map(normalizeSuccessRateItem).filter(Boolean);
}
...
export function normalizeClientShowcase(entry) {
  ...
  const id = raw.documentId ?? raw.id;
  const logo = raw.clientLogo ?? raw.logo ?? raw.thumb ?? raw.thumbnail;
  const logoUrl = strapiMediaUrl(logo);
  const logoAttrs =
    logo && typeof logo === "object" ? (logo.attributes ?? logo) : null;
  const backgroundUrl =
    firstCoverUrl(raw) ??
    firstContentImageUrl(raw.content) ??
    logoUrl ??
    null;
...
export async function getClientShowcases(limit = 24) {
  ...
  const json = await fetchStrapiCollection(plural, showcaseFetchQuery({
    sort: "publishedAt:desc",
    "pagination[pageSize]": String(limit),
  }));
  return normalizeClientShowcasesResponse(json).filter(
    (card) => card.backgroundSrc && card.thumbSrc,
  );
}
